...
module.exports = {
  ...
  make_jiff: function (base_instance, options) {
    ...
    jiff.maxBatchSize = (options.maxBatchSize == null) ? 200 : options.maxBatchSize;
    ...
    jiff.restful.handlers = {
      ...
      open: jiff.open,
      custom: jiff.custom,
      triplet: function (computation_id, from_id, msg) {
        return jiff.triplet(computation_id, from_id, JSON.stringify(msg));
      },
      number: function (computation_id, from_id, msg) {
        return jiff.number(computation_id, from_id, JSON.stringify(msg));
      },
      free: jiff.free
    };

    // Functionality managing mailboxes
    jiff.restful.dump_mailbox = async function (computation_id, party_id) {
      try {
        ...
        var messages = [];
        var count = Math.min(mailbox.length, jiff.maxBatchSize);
        ...
        jiff.restful.pending_messages[computation_id][party_id] = { tag: tag, store_id: count };

        return { success: true, ack: tag, messages: messages };
      } catch (error) {
    ...
    jiff.restful.poll_initialize = async function (msg) {
      ...
      if (initialization['party_id'] === 's1') {
        return { success: false, error: 'invalid party id'};
      }

      var output = await jiff.initialize_party(msg['computation_id'], initialization['party_id'], initialization['party_count'], initialization);
      if (!output.success) {
        return output;
      }
    ...
    jiff.restful.poll_handle_message = async function (computation_id, from_id, messages) {
      var promises = [];
      for (var i = 0; i < messages.length; i++) {
        ...
        // handle message
        var output = await jiff.restful.handlers[label](computation_id, from_id, payload);
        ...
        if (output.message != null) {
          promises.push(jiff.hooks.put_in_mailbox(jiff, label, output.message, computation_id, from_id));
        }
      }
    ...
    app.post('/poll', async function (req, res) {
      ...
      // Execute start hooks
      try {
        msg = await jiff.execute_array_hooks('beforeOperation', [jiff, 'poll', msg['computation_id'], msg['from_id'], msg], 4);
      } catch (err) {
        return res.json({ success: false, error: typeof(err) === 'string' ? err : err.message });
      }

      // First: attempt to initialize if needed.
      var output = await jiff.restful.poll_initialize(msg);
      ...
      if (from_id === 's1') {
        return res.json({ success: false, error: 'invalid party id'});
      }
      ...
      output = await jiff.restful.poll_handle_message(computation_id, from_id, msg.messages);
      if (!output.success) {
        return res.json(output);
      }

      // Fourth: dump mailbox and encrypt.
      var dumped = await jiff.restful.dump_mailbox(computation_id, from_id);
      if (!dumped.success) {
        return res.json(dumped);
      }

      response = Object.assign(response, dumped);

      // Execute end hooks
      response = await jiff.execute_array_hooks('afterOperation', [jiff, 'poll', computation_id, from_id, response], 4);
